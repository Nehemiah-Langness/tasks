...
export function Today() {
    ...
    const pastDue = useMemo(() => tasks?.filter((x) => !x.nextDueDate || x.nextDueDate.valueOf() < now.valueOf()) ?? [], [now, tasks]);

    const dueToday = useMemo(() => tasks?.filter((x) => x.nextDueDate && x.nextDueDate.valueOf() === now.valueOf()) ?? [], [now, tasks]);
    ...
    const tasksCompleted = useMemo(() => tasks?.filter((x) => x.task.lastCompleted && x.task.lastCompleted >= now.valueOf()), [now, tasks]);
                ...
                {dueToday.length ? (
                    ...
                    <div className='px-2 py-4 d-flex justify-content-center align-items-center border border-success align-self-stretch my-3 rounded-2 bg-success-subtle position-relative overflow-hidden'>
                        ...
                        <div className='fs-150 position-relative text-center' style={{ zIndex: 1 }}>
                            ...
                            {!!tasksCompleted?.length && (
                                <div className='fs-140'>You have finished {tasksCompleted?.length} tasks today!</div>
                            )}
                        </div>
                    </div>
                )}
...
export function TaskRow({ task, dueDate, pastDue }: { task: Task; dueDate?: number; pastDue?: boolean }) {
    ...
    return (
        <div className='d-flex gap-3 align-items-center'>
            ...
            <div className='d-flex gap-2 align-items-center px-3 flex-wrap'>
                {task.lastCompleted && (
                    <div style={{ paddingTop: 2, paddingBottom: 2 }} className='bg-success px-3 fs-80 rounded-pill text-white'>
                        Last Completed on{' '}
...
function CompleteButton({ onClick }: { onClick: () => void }) {
    ...
    return (
        <button className='btn btn-link text-decoration-none link-success d-flex align-items-center' onClick={() => setCompleted(true)}>
            <ButtonIcon />
        </button>
    );
}
