...
export function TaskRow({ task, dueDate }: { task: Task; dueDate?: number }) {
    ...
    const nextDueDate = useMemo(() => {
        ...
        let nextDueDate = dueDate ? new Date(dueDate) : getNextPassingDate(toDay(task.startDate), task.filters, true);
        while (nextDueDate && nextDueDate.valueOf() < now) {
            nextDueDate = getNextPassingDate(nextDueDate, task.filters);
        }
        return nextDueDate ? formatDate(nextDueDate, { date: true, day: true }) : 'Never';
    }, [dueDate, task.filters, task.startDate]);
    return (
        <div>
            <div className='h5 fw-normal d-flex align-items-center'>
                <button disabled={task.id.startsWith('pool-')} className='btn btn-link' type='button' onClick={() => load(task)}>
                    <FontAwesomeIcon icon={faPenToSquare} />
                </button>

                {task.description}
                <button disabled={task.id.startsWith('pool-')} className='btn btn-link link-danger' type='button' onClick={deleteTask}>
            ...
            <div className='d-flex gap-2 align-items-center px-3 flex-wrap'>
                ...
                {task.lastCompleted && (
                    <div style={{ paddingTop: 2, paddingBottom: 2 }} className='bg-success px-3 fs-80 rounded-pill text-white'>
                        {formatDate(new Date(task.lastCompleted), { date: true, day: true })}
                    </div>
                )}
